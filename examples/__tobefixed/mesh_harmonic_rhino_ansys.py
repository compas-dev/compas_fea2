...
from compas_fea2.backends import ansys
from compas_fea2.backends.ansys import get_nodes_elements_from_result_files
from compas_fea2.backends.ansys import get_harmonic_data_from_result_files
...
from compas_fea2.backends.ansys import HarmoniStepBase
from compas_fea2.backends.ansys import HarmoniPointLoadBase
...
def harmonic(mesh, pts, lpts, freq_range, freq_steps, damping, path, filename):
    ...
    load = HarmoniPointLoadBase(name='harmonic_load', nodes=nkeys, z=-1)
    s.add_load(load)

    # add modal step -----------------------------------------------------------
    step = HarmoniStepBase(name='harmonic_analysis', displacements=['supports'],
