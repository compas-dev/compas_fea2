...
""".format(self.model.name)
    ...
    def _generate_materials_sections(self):
        material_data = "\n".join(mat._generate_jobdata() for mat in self.model.materials)
        section_data = "\n".join(sec._generate_jobdata() for sec in self.model.sections)
        parsed_data = "\n\n".join((material_data, section_data))
        module_data = self._generate_aqua_module(parsed_data)
        return module_data

    def _generate_geometry(self):
        node_data = "\n".join(node._generate_jobdata() for node in self.model.nodes)
        beam_data = "\n".join(beam._generate_jobdata() for beam in self.model.beams)
    ...
    def _generate_boundary_conditions(self):
        bcs_data = "\n".join(bc._generate_jobdata() for bc in self.model.boundary_conditions)
        module_data = self._generate_sofimsha_module(bcs_data, continue_model=True, name="constraints")
        return module_data

    def _generate_load_cases(self):
        load_cases_data = (lc._generate_jobdata() for lc in self.model.load_cases)
    ...
    def _generate_analyses(self):
        analyses_data = (al._generate_jobdata() for al in self.model.analyses)
    ...
    def _generate_stresses(self):
        stress_data = "\n".join(st._generate_jobdata() for st in self.model.stresses)
    ...
    def write_to_file(self, path):
        ...
        try:
            ...
            with open(filepath, 'w') as f:
                f.writelines(self._generate_jobdata())
